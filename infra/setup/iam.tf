...
#############################################################
# Policy for Terraform backend to S3 and Dynambo DB access #
#############################################################

# Defining the IAM policy document for Terraform backend access
# It is also possible that we inject a json file insted of this inline document
data "aws_iam_policy_document" "tf_backend" {
  ...
  statement {
    ...
    # NOTE: Since these resources were created manually, we need to specify the arn
    resources = [
...
# Create the IAM policy resource for Terraform backend access
# This policy allows the user to access S3 and DynamoDB for Terraform state management
...
data "aws_iam_policy_document" "ecr" {
  ...
  statement {
    ...
    # NOTE: These resources are created by terraform so can be referenced directly
    resources = [
...
resource "aws_iam_policy" "service_linked_role" {
  name        = "${aws_iam_user.cd.name}-service-linked-role"
  description = "Allow user to create service-linked roles"
  policy      = data.aws_iam_policy_document.service_linked_role.json
}

resource "aws_iam_user_policy_attachment" "service_linked_role" {
  user       = aws_iam_user.cd.name
